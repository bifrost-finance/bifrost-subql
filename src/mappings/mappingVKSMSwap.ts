...
import { VtokenBncSwap, VtokenMovrSwap, VtokenSwap, VtokenSwapRatio, } from "../types";
...
export async function handleVPHASwap(event: SubstrateEvent): Promise<void> {
    const {
        event: {
            data: [owner, recipient, swap_path, balances],
        },
    ...
    await Promise.all(
        new Array(swap_path_obj.length - 1).fill("").map(async (_, key) => {
            ...
            const isVPHA_PHA =
                (asset0.name === "vPHA" && asset1.name === "PHA") ||
                (asset0.name === "PHA" && asset1.name === "vPHA");


            if (isVPHA_PHA) {
                const entity = new VtokenSwapRatio("vPHA_PHA");
