...
export async function zenlinkAssetSwap(event: SubstrateEvent): Promise<void> {
  ...
  await Promise.all(new Array(swap_path_obj.length - 1).fill('').map(async (_, key) => {
    ...
    const amount_balance = asset0.coin_id ? toUnitToken(balances_obj[key], asset0.decimal) : toUnitToken(balances_obj[key + 1], asset1.decimal)
    const entity = new ZenlinkLiquidityCalculation(blockNumber.toString() + '-' + event.idx.toString() + '-' + key.toString());
