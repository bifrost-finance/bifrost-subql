...
const isFundEvent = (eventType:string):boolean => {
  if (eventType === 'Created' || eventType === 'Dissolved' || eventType === 'Withdrew' || eventType === 'Edited'
      || eventType === 'Success' || eventType === 'Failed' || eventType === 'Retired' || eventType === 'End'){
    return true
  }
  return false;
}
...
const isContributionEvent = (eventType:string):boolean => {
  if (eventType === 'Contributing' || eventType === 'Contributed' || eventType === 'ContributeFailed'
      || eventType === 'Issued' || eventType === 'Redeemed' || eventType === 'Refunded') {
    return true
  }
  return false;
}
...
export async function salp(block: SubstrateBlock): Promise<void> {
  ...
  let paraId;
  const salpEvents = block.events.filter(e => (e.event.section === 'salp' || e.event.section === 'salpLite')) as SubstrateEvent[];
...
export async function salpContribution(block: SubstrateBlock): Promise<void> {
  ...
  for (let salpEvent of salpEvents) {
    ...
    if(isContributionEvent(method)) {
      ...
      if(method === 'Contributing' || method === 'Contributed' || method === 'ContributeFailed' || method === 'Issued'){
